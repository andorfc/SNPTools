...
# Variable list (the INFO column is stored verbatim in the HDF5 and passed
# straight through, so all annotation keys -- GENEMODEL, TYPE, EFFECT, SUB,
# MQ, CVP, MAXR2, MAF, plantcad1/2_score, ESM1/2/3_score -- survive unchanged)
...
def gt_string(val):
    """Robustly map an HDF5 genotype code to a VCF genotype string."""
    try:
...
with h5py.File(hdf5_file_path, 'r') as hdf5_file:
    ...
    # Convert 'POS' data from byte strings/objects to integers
    try:
    ...
    # Only read columns that actually exist in this file; the rest are filled
    # with missing genotypes ('./.') below so a stray/foreign accession id in
    # the request never aborts the whole query.
...
if len(variant_data['POS']) == 0:
    ...
    with open(output_vcf_path, 'w') as vcf_file:
        ...
        if "maizegdb" in hdf5_file_path:
            write_common_info_header()
            vcf_file.write("##source=MaizeGDB\n")
        ...
        if "nam" in hdf5_file_path:
            write_common_info_header()
            vcf_file.write("##source=MaizeGDB+NAM\n")
        ...
        for i in range(len(variant_data['POS'])):
            ...
            # genotype per requested accession, in request order; missing -> ./.
