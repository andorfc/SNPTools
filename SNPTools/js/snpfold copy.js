...
(function () {
  ...
  const FD = {
    ...
    dataset: null, sec: true,   // sec = show PlantCAD2/ESM2/ESM3 (MaizeGDB 2026 only)
  ...
  function impactHex(s){ const lo=-12,hi=4,t=Math.max(0,Math.min(1,(s-lo)/(hi-lo)));
    const r=t<.5?220:Math.round(220*(1-(t-.5)*2)), g=t<.5?Math.round(180*t*2):170; return `rgb(${r},${Math.max(50,g)},55)`; }
  ...
  async function render(page){
    ...
    if (S.foldGene){ FD.gene = S.foldGene; S.foldGene = null; FD.selId = null; FD.openCarrier = null; }
    FD.viewer = null;
    FD.sec = Data.hasSecondaryScores(FD.dataset);
    injectCSS();
    ...
    const s = FD.struct;
    const meanP = (s.plddt.reduce((a,b)=>a+b,0)/s.length).toFixed(0);

    page.innerHTML = searchBar() + `<div class="loading" style="padding:48px;text-align:center">
      <div class="spinner"></div><div>Loading coding variants for ${s.gene}…</div></div>`;
    try {
      const [variants, fn] = await Promise.all([
        Data.queryFoldVariants(FD.gene),
        Data.geneFunction(FD.gene, FD.dataset).catch(()=>null),   // whole-panel carriers
      ]);
    ...
    catch (e){ FD.variants = []; FD.carriers = null; }
    ...
    page.innerHTML = searchBar() + `
      ...
      <div class="tbl-wrap" style="max-height:none"><table class="vcf">
        <thead><tr>
          ...
          <th class="num">Local pLDDT</th><th>Structure</th><th class="num">PlantCAD</th><th class="num">ESM</th>
          ${FD.sec?'<th class="num">PlantCAD2</th><th class="num">ESM2</th><th class="num">ESM3</th>':''}
  ...
  function rowHTML(v){
    ...
    return `<tr class="fold-row ${on?'sel':''}" onclick="FOLD.select('${v.id}')">
      ...
      ${FD.sec?`<td class="num">${scoreCell(v.plantcad2)}</td><td class="num">${scoreCell(v.esm2)}</td><td class="num">${scoreCell(v.esm3)}</td>`:''}
  ...
  function carrierRow(v, cr){
    ...
    const homs = cr.carriersHom.slice(0,60).map(id=>chip(id,'hom')).join('');
    const hets = cr.carriersHet.slice(0,60).map(id=>chip(id,'het')).join('');
    return `<tr class="fn-carriers"><td colspan="${foldCols()}">
      <div class="fn-cwrap">
        <div><div class="fn-k">Homozygous ${v.consClass==='lof'?'(candidate knockouts)':''} · ${cr.carriersHom.length}</div>
          <div class="fn-chips">${homs||'<span class="muted">none</span>'}${cr.carriersHom.length>60?` <span class="muted">+${cr.carriersHom.length-60} more</span>`:''}</div></div>
        <div style="margin-top:8px"><div class="fn-k">Heterozygous · ${cr.carriersHet.length}</div>
          <div class="fn-chips">${hets||'<span class="muted">none</span>'}${cr.carriersHet.length>60?` <span class="muted">+${cr.carriersHet.length-60} more</span>`:''}</div></div>
      </div></td></tr>`;
  }
  ...
  function ctxHTML(){
    ...
    return `
      ...
      <div class="ctx-grid">
        ...
        <div class="ck"><div class="kk">AI scores</div><div class="vv mono">${v.plantcad==null&&v.esm==null?'n/a':`PlantCAD ${v.plantcad==null?'—':v.plantcad} · ESM ${v.esm==null?'—':v.esm}`}${FD.sec?` · PlantCAD2 ${v.plantcad2==null?'—':v.plantcad2} · ESM2 ${v.esm2==null?'—':v.esm2} · ESM3 ${v.esm3==null?'—':v.esm3}`:''}</div></div>
      </div>
  ...
  function applyStyle(){
    ...
    if (FD.showVar){
      FD.variants.forEach(vr=>{
        ...
        v.addStyle({ resi:vr.resi }, { sphere:{ scale: vr.id===FD.selId?.7:.45, color: col } });
      });
    }
    v.render();
  }
