...
function colorGenome(genome) {
    if (genome == "B73_v5") {
        ...
        return '#455edd'; //non SS
    } else if (genome == "PH207_CAAS") {
        return '#a807ed'; //Iodent
    } else if (genome == "PH207_UIUC") {
        return '#a807ed'; //Iodent
        ...
        return '#30c727'; //Tropical
    } else if (genome == "CML457") {
        return '#30c727'; //Tropical
    } else if (genome == "CML530") {
        return '#30c727'; //Tropical
    } else if (genome == "CML459") {
        return '#30c727'; //Tropical
...
//This function toggles if the IDS for the Species or Proteins are show in the Zoomed In heatmap view in the pan-genome view
function switchView() {
    // Get the value of the dropdown menu
    let selectorValue = document.getElementById("viewSelector").value;

    // Get all span elements with the respective classes
    let speciesOptions = document.querySelectorAll(".species_option");
    let proteinOptions = document.querySelectorAll(".protein_option");

    // Check the value of the dropdown and adjust the visibility accordingly
    if (selectorValue === "Species") {
        speciesOptions.forEach(span => span.style.display = "inline");
        proteinOptions.forEach(span => span.style.display = "none");
        view_type = "Species";
    } else if (selectorValue === "Protein") {
        speciesOptions.forEach(span => span.style.display = "none");
        proteinOptions.forEach(span => span.style.display = "inline");
        view_type = "Protein";
    }
}
